...
8. **Sausalito City Events** — fetch sausalito.gov/our-city/calendar-of-events directly. Confirmed (2026-07) that the entire sausalito.gov domain returns HTTP 403 Forbidden via WebFetch on every path/variant tried. Treat as blocked and report via attestation rather than retrying further — needs a different access method (e.g. a logged-in/rendered browser fetch) to ever complete; flag to Alexandra periodically until resolved.
...
38. **Sausalito Public Library** — fetch sausalitolibrary.org/kids/library-calendar (renders a cached month), then chain the "Next Month >" link (`-curm-{M}/-cury-{Y}`) forward through each month in the sweep window — step month by month, can't jump directly to a future month. Capture "KIDS SUMMER READING EVENT" one-offs. Do NOT use the /kids-teens/summer-reading-2026 page (image-only) or the old /kids/children-s-programs path (stale cache) — confirmation-only, not the fetch route. **Confirmed (2026-07)**: every URL pattern above, including the documented curm/cury route, returned HTTP 403 Forbidden. If still blocked, report as blocked via attestation. Do NOT substitute a web-search-located event page as a workaround — one such page found this way was dated 2024 despite appearing in a fresh search, a stale-content trap.
