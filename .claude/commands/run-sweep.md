...
1. **Marin Mommies — Weekly Weekend Roundup post** (marinmommies.com) — **THIS IS A REQUIRED, PRIMARY SOURCE EVERY SWEEP — do not skip it.** Marin Mommies publishes a hand-curated "Marin & Bay Area Weekend Family Fun for [dates]" roundup post each week (URL pattern: `marinmommies.com/marin-bay-area-weekend-family-fun-<month>-<start>%E2%80%93<end>`, e.g. `.../marin-bay-area-weekend-family-fun-july-24%E2%80%9326`). **This editorially-curated roundup contains family events that never appear on the raw per-day /calendar/ pages** — confirmed 2026-07: the per-day-calendar-only approach MISSED Cricket & the Wren Circus, Lucky Break concert, a China Camp Junior Ranger Nature Ramble, and more, all of which were on that week's roundup post. Fetch the roundup post for EVERY weekend in the sweep window and review every Marin event on it, cross-checking each against `events.json`. **Year-verification gate (mandatory):** these blog-post URLs have occasionally served stale slug-cached content from a prior year (a "July 8-10" slug once returned 2022 content, "July 11-13" returned 2025). So the FIRST thing to do on each roundup page is confirm the explicit year/full dates printed on the page match the target year. If the year is correct → review the whole page as a primary source. If the year is wrong/stale → note it in attestation and fall back to the per-day pages for that weekend. Do NOT use the old rule of avoiding these pages outright — that caused real misses. ALSO fetch the per-day pages `marinmommies.com/calendar/YYYY-MM-DD` for each Fri/Sat/Sun (live, reliably dated) as a complementary source — use BOTH, since each surfaces items the other misses. Confirm specifics via per-event pages at marinmommies.com/calendar/[event-slug]. Cross-check day-of-week labels against a real calendar — the site has mislabeled days before. Note: `www.marinmommies.com` resolves; the bare `marinmommies.com` domain sometimes fails DNS — use the `www.` host.
