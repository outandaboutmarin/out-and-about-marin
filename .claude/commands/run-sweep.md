...
1. **Marin Mommies — Weekend Post** (marinmommies.com) — Direct fetch of the dated post is robots-blocked. Search-around to exhaustion: one query per Marin town (Tiburon, Mill Valley, San Rafael, Novato, Fairfax, Sausalito, San Anselmo, Larkspur, Ross) for the UPCOMING Fri–Sun weekend (not the current week), repeating until Friday+Saturday+Sunday sections have all surfaced (realistic floor ~12–15 events, not 6). Confirm specifics via per-event pages at marinmommies.com/calendar/[event] (these ARE fetchable). Cross-check day-of-week labels against a real calendar — the site has mislabeled days before.
...
8. **Sausalito City Events** — fetch sausalito.gov/our-city/calendar-of-events directly.
...
15. **MALT (Marin Agricultural Land Trust)** — fetch malt.org/malt_events/ directly, look for farm tours/hikes/community days in West Marin.
16. **Megan Schoenbohm — Upcoming Shows** — fetch musictimewithmegan.com/copy-of-classes directly (page titled "UPCOMING SHOWS"). Marin venues only: Marin Country Mart (Larkspur), Pelican Inn (Muir Beach), Novato Farmers Market. Do NOT add Bon Air entries — removed at Alexandra's direction.
...
25. **MCFL — Civic Center** (San Rafael) — fetch marinlibrary.org/locations/mb/.
26. **MCFL — Corte Madera** — fetch marinlibrary.org/locations/mm/. Check closure status (2nd Refresh closure Jul 6–Sep 2, 2026 — reopens Sep 3).
27. **MCFL — Fairfax** — fetch marinlibrary.org/locations/mf/.
28. **MCFL — Marin City** — fetch marinlibrary.org/locations/mc/.
29. **MCFL — Novato** — fetch marinlibrary.org/locations/mn/. Check closure status (closed Jun 17–Aug 18, 2026, reopens Aug 19).
30. **MCFL — South Novato** — fetch marinlibrary.org/locations/mn/ (same branch page as Novato).
31. **MCFL — Point Reyes** — fetch marinlibrary.org/locations/mp/.
32. **MCFL — Bolinas** — check marinlibrary.bibliocommons.com series URLs for existing program IDs (175/176); very small branch.
...
38. **Sausalito Public Library** — fetch sausalitolibrary.org/kids/library-calendar (renders a cached month), then chain the "Next Month >" link (`-curm-{M}/-cury-{Y}`) forward through each month in the sweep window — step month by month, can't jump directly to a future month. Capture "KIDS SUMMER READING EVENT" one-offs. Do NOT use the /kids-teens/summer-reading-2026 page (image-only) or the old /kids/children-s-programs path (stale cache) — confirmation-only, not the fetch route.
