...
6. **Osher Marin JCC** — **`/events/` IS NOT ENOUGH. It is not the source of record for JCC family programming and never was.** Fetching only that page is a documented miss (2026-08-27, caught by Alexandra, not by the process): `/events/` carried 13 items that sweep, **none of which was a JBaby family program**, and the sweep reported "none new" while **MusicBabies — a brand-new fall program running at BOTH sites — went unproposed**. The old wording here said absence from that calendar was "weak evidence"; that was too soft and it got treated as sufficient. The calendar is an adult-events feed in practice: book clubs, lectures, Israeli dance, musical matinees.
    ...
    **PRE-REGISTERED WEEKLY CLASSES ARE OUT OF SCOPE — A STANDING RULE, NOT A ONE-OFF CALL (settled 2026-09-03).** Alexandra: *"Side by Side is a weekly class that you have to sign up for. We don't usually include pre-registered weekly classes on this app."* The test is **can a family just turn up?** A drop-in session is in scope even when it charges at the door; a course you enrol in for a term is not, however family-oriented it looks. This is the same line the JBaby paragraph below draws when it says most of those programmes qualify *because* they allow drop-in, and the same line source #1's venue-drop-in exclusion draws from the other direction. Apply it to any source, not just the JCC.
...
11. **Ronnie's Awesome List** — fetch the current Marin kids roundup page, plus ronniesawesomelist.com/free-music-marin. Newsletter drops Thursdays — check on Friday sweeps.
    ...
    **Rules for this source, and they generalise to any undated roundup:**
    - **Never take a date from this page.** Use it to learn that an event *exists*, then get the date from the organiser.
    - **Re-derive the weekday from every date** (rule 14). A season whose dates land on a different weekday than the source describes is a wrong-year season, and that single check would have caught both incidents.
...
41. **Larkspur Library** — **THE LIBRARY RUNS A LIBCAL. FETCH THAT FIRST (added 2026-09-10).**

    ⚠ **This entry used to say only that Larkspur "posts essentially NOTHING" and that absence was not evidence. That was half the picture and it cost real coverage.** The **city** calendar is indeed empty — still verified so on 2026-09-10 for October. But the library maintains a full **LibCal** that no previous sweep had ever fetched, because it is reachable from no page this checklist named. It was found only because Marin Mommies linked one of its event pages.

    **Fetch route — same shape as source #37 (Mill Valley), one navigation per month:**
    `larkspurlibrary.libcal.com/calendar?cid=-1&cal=-1&inc=0&d=YYYY-MM-DD&t=m`
    It is JS-rendered, so use the browser and read `.s-lc-mc-evt` out of the DOM. **Both of #37's traps apply here identically** — the grid's visible text carries only day numbers, and each week's row holds 14 `td`s of which only the 7 `.s-lc-desktop-only` ones may be paired with the date row.

    **What one pass produced on 2026-09-10** (Sept + Oct): four new candidates that no other source carried — a children's author event, a world-percussion session, a Halloween magic show and Friday Crafternoons — plus a **CANCELLED** Sept 25 storytime and the discovery that **id 15 was over-generating**: the Friday 10 AM slot alternates between Storytime and Cuentos con Ritmo, so the record was advertising Storytime on Fridays it does not run. None of that is visible on the city calendar or the Programs pages.

    **The generalisable lesson: "this venue publishes nothing" is a claim with a shelf life.** It was true of the route we knew about and false of the venue. When a source is written off, record *which route* was checked — not that the venue is silent.

    The city pages below remain useful for programme blurbs and closures, and the old note about the moved path still holds: `/185/Library` now 301s to `larkspur.ca.gov/185/Library` which returns a CivicEngage **Custom404** — that is a moved page, *not* a bot block, so don't conclude "blocked" from it. Current paths:
