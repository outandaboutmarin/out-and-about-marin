...
**Health**: `events.json` is 343 events / max id 836, all committed and pushed. Live site is current. **One known user-facing problem**: 39 *non-library* expired one-offs are still rendering (see open item 1 below) — found during the library audit but left alone, since batch deletions need Alexandra's sign-off and they're outside that audit's scope.

**AWAITING REVIEW — open item 28, the library audit. Fetching and diffing are COMPLETE; all 16 branches were read for Aug 13 – Sep 20.** Scope was all **124 library-hosted records**, not just the 94 typed `Library`. The deliverable is `OAA maintence and content/library_audit_2026-08-13_review.xlsx` — four sheets: Weekly Sweep (11 new candidates), **Record Fixes (22 proposed corrections, a new sheet type)**, Questions (7, each with a recommendation), Attestation Log (16 branches). Alexandra fills `Decision` and `YOUR ANSWER`; applying it needs `/process-sweep` **extended to read the Record Fixes sheet**, which it does not do yet. Full working notes and the per-branch live inventory are in `library_audit_2026-08-12_progress.md`.

Already shipped from the audit (`0870cab`, `a018e5e`): 21 expired one-offs deleted; **four duplicate pairs removed** (766/799, 767/800, 768/801, 34/627 — every one was rendering the same event twice on the public site, see the rewritten rule 13); ids 42/43 fixed, each having held two conflicting `Reopens` dates so the badge showed Aug 25 instead of Sep 3 (**second occurrence of the rule 9b bug after id 8 — this failure mode recurs, check for it every sweep**); stale June reopening sentences cleared on ids 6, 7, 40, 48, 50.
